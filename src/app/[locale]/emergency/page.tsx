...
export default function EmergencyPage() {
  ...
  const getRelativeTime = (dateString: string) => {
    ...
    if (diff < 1) return t('initiating');
    if (diff < 60) return `${diff} ${t('minSuffix')}`;
    return `${Math.floor(diff / 60)} hours`;
  };
    ...
    <div className="container mx-auto px-4 md:px-8 py-10 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div className="space-y-4">
          ...
          <p className="text-xl text-muted-foreground font-medium animate-in fade-in slide-in-from-left duration-700 delay-100 italic">{t('emergencySub')}</p>
        ...
        >
          <Plus className="h-6 w-6 mr-3" /> {t('broadcastSos')}
        </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1 animate-in slide-in-from-left duration-700">
             <Card className="glass-card border-none overflow-hidden rounded-[2.5rem] shadow-2xl border-2 border-primary/20">
               <CardHeader className="bg-primary p-8 text-white relative overflow-hidden">
                 ...
                 <CardTitle className="text-2xl font-black italic tracking-tight relative z-10">{t('broadcastEmergency')}</CardTitle>
                 <CardDescription className="text-white/80 font-medium relative z-10">{t('notifyDonors')}</CardDescription>
                ...
                <CardContent className="space-y-6 p-8">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t('bloodGroup')}</label>
                      <Select required>
                        <SelectTrigger className="h-14 rounded-2xl border-2 focus:ring-primary font-bold text-lg"><SelectValue placeholder={t('selectGroup')} /></SelectTrigger>
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t('unitsMl')}</label>
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t('medicalCenter')}</label>
                    <div className="relative">
                      <Hospital className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <Input placeholder={t('hospitalPlace')} className="pl-12 h-14 rounded-2xl border-2 focus:ring-primary font-bold text-lg" required />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t('directLocation')}</label>
                    <div className="relative">
                      <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <Input placeholder={t('locationPlace')} className="pl-12 h-14 rounded-2xl border-2 focus:ring-primary font-bold text-lg" required />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t('contactIntel')}</label>
                    <div className="relative">
                      <Phone className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                      <Input placeholder={t('phonePlace')} className="pl-12 h-14 rounded-2xl border-2 focus:ring-primary font-bold text-lg" required />
                    </div>
                  </div>

                </CardContent>
                <CardFooter className="flex flex-col gap-4 p-8 pt-0">
                  <Button type="submit" className="w-full bg-primary h-16 font-black text-xl rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all">{t('establishBroadcast')}</Button>
                  <Button type="button" variant="ghost" className="w-full font-black text-xs uppercase tracking-widest text-muted-foreground hover:text-primary" onClick={() => setShowForm(false)}>{t('abortMission')}</Button>
                </CardFooter>
        ...
        <div className={showForm ? "lg:col-span-2" : "lg:col-span-3"}>
          <div className="space-y-8">
            <div className="flex items-center gap-6 mb-8">
              <div className="flex items-center gap-2 px-4 py-2 bg-red-500/10 border border-red-500/20 rounded-full">
                ...
                <span className="text-xs font-black text-red-500 uppercase tracking-widest">{t('liveFeed')}</span>
              </div>

              <Badge className="bg-primary/5 text-primary border-primary/20 py-2 px-6 rounded-full font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-primary/5">{t('nationalActive')}</Badge>

              <div className="hidden md:flex items-center gap-2">
                <RefreshCw className="h-3 w-3 text-muted-foreground animate-spin" style={{ animationDuration: '4s' }} />
                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{t('realtimeSync')}</span>
              </div>
                  ...
                  <CardContent className="p-10 relative z-10">
                    <div className="flex flex-col md:flex-row gap-12">
                      <div className="flex-shrink-0 flex flex-col items-center justify-center bg-white/5 rounded-[2rem] p-8 min-w-[160px] border border-white/10 group-hover:bg-primary/20 transition-all duration-500 shadow-inner relative overflow-hidden">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 relative z-10">{t('bloodGroup')}</span>
                        ...
                        <div className="flex flex-col items-center mt-4 relative z-10">
                           <span className="text-sm font-black text-white uppercase tracking-widest">{request.units} {t('unitsCount')}</span>
                           {request.urgency === 'Critical' && (
                             <Badge className="bg-primary text-white text-[8px] font-black mt-2 animate-bounce tracking-tighter px-2">{t('requiredNow')}</Badge>
                           )}
                        </div>
                      ...
                      <div className="flex-grow space-y-8">
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                                ...
                                <div className={cn(
                                  ...
                                  'bg-yellow-500/20 border-yellow-500 text-yellow-500'
                                   ...
                                   {request.urgency} {t('severityLabel')}
                          ...
                          <div className="flex flex-col items-end gap-4 min-w-[200px]">
                            <div className="flex flex-col items-end">
                                <p className="text-[10px] font-black text-slate-500 flex items-center gap-2 justify-end uppercase tracking-[0.2em] mb-1">
                                  <Clock className="h-3.5 w-3.5" /> {t('posted')} {getRelativeTime(request.createdAt)}
                                </p>
                                ...
                                <div className={cn(
                                  ...
                                  expiresIn < 15 ? "bg-red-500/10 border-red-500 text-red-500 animate-pulse" : "bg-white/5 border-white/10 text-slate-400"
                                   ...
                                   ⏱️ {t('expiringIn')} {expiresIn} {t('minSuffix')}
                                </div>
                            ...
                            <Badge className="bg-green-500/10 text-green-400 border border-green-500/30 py-2 px-5 font-black text-[10px] uppercase tracking-widest rounded-xl transition-all hover:bg-green-500/20">
                               <ShieldCheck className="h-4 w-4 mr-2" /> {t('govVerified')}
                            </Badge>
                        ...
                        <div className="flex flex-wrap items-center gap-6 pt-8 border-t border-white/10">
                          <Button className="btn-premium bg-white text-slate-900 hover:bg-primary hover:text-white font-black px-12 rounded-xl h-14 transition-all shadow-xl text-sm">
                            <Phone className="h-5 w-5 mr-3 animate-wiggle" /> {t('connectHospital')}
                          </Button>
                          ...
                          <Button variant="outline" className="btn-premium font-black border-white/10 text-white hover:bg-white/5 rounded-xl h-14 px-10 uppercase tracking-widest text-[10px] transition-all hover:border-primary/40 group">
                            <Share2 className="h-5 w-5 mr-3 group-hover:text-primary transition-colors" /> {t('shareSos')}
                          </Button>
                          ...
                          <div className="ml-auto hidden xl:flex items-center gap-3 text-slate-500">
                             ...
                             <span className="text-[10px] font-black uppercase tracking-widest">3 {t('donorsResponding')}</span>
          ...
          <Card className="mt-16 glass-card border-none rounded-[3rem] p-12 text-center animate-in fade-in zoom-in duration-700 delay-500">
            <h4 className="text-3xl font-black italic tracking-tight mb-4 text-foreground">{t('missionCritical')}</h4>
            <p className="text-lg text-muted-foreground mb-8 max-w-2xl mx-auto font-medium italic">{t('backbone')}</p>
            <Button size="lg" className="h-16 px-12 bg-white text-slate-900 hover:bg-primary hover:text-white font-black text-xl rounded-2xl transition-all shadow-2xl hover:scale-105" asChild>
              <Link href="/register">{t('enlistHero')}</Link>
            </Button>
          </Card>
          ...
          <TooltipTrigger asChild>
             ...
             >
               <div className="flex flex-col items-center">
                  ...
                  <span className="text-[10px] font-black uppercase mt-1">{t('newSos')}</span>
               </div>
