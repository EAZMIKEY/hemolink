...
import { Search, MapPin, Droplet, Phone, Mail, Calendar, Info } from 'lucide-react';
...
export default function SearchPage() {
  ...
  const filteredDonors = MOCK_DONORS.filter(donor => {
    const matchesGroup = bloodGroup ? (showCompatibility ? getCompatibleDonors(bloodGroup as BloodGroup).includes(donor.bloodGroup) : donor.bloodGroup === bloodGroup) : true;
        ...
        <div className="lg:col-span-3">
          {filteredDonors.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                          <MapPin className="h-3 w-3" /> {donor.city} • {donor.distance || '0.5 km'} away
                        </p>
                      </div>
                    </div>
                  ...
                  <CardContent className="p-4 pt-4 space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div className="space-y-1">
                        <span className="text-muted-foreground block">Last Donated</span>
                        <span className="font-semibold flex items-center gap-1">
                          <Calendar className="h-3 w-3" /> {new Date(donor.lastDonationDate).toLocaleDateString()}
                        </span>
                      </div>
                      <div className="space-y-1">
                        <span className="text-muted-foreground block">Status</span>
                        <span className={`font-semibold flex items-center gap-1 ${donor.isAvailable ? 'text-green-600' : 'text-amber-600'}`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${donor.isAvailable ? 'bg-green-600' : 'bg-amber-600'}`} />
                          {donor.isAvailable ? 'Available Now' : 'Busy'}
                        </span>
                      </div>
                    </div>
                    
                    <div className="flex gap-2 pt-2">
                      <Button size="sm" className="flex-1 bg-primary font-bold">
                        <Phone className="h-4 w-4 mr-2" /> Call Now
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1 font-bold border-secondary text-secondary">
            ...
            <div className="bg-card border rounded-2xl p-20 text-center">
              ...
              <p className="text-muted-foreground max-w-sm mx-auto">Try broadening your search criteria or checking compatible blood groups.</p>
              <Button variant="link" className="text-primary font-bold mt-4" onClick={() => {setBloodGroup(''); setCity('');}}>
                Clear all filters
              </Button>
            </div>
          )}
